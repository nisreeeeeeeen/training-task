...
class TfIdf(object):
    ...
    def crawl_fox_news(self, fox_url):
        """
        提取指定foxnews网页链接的新闻标题和正文，去掉正文中的特殊符号，返回标题的字符串和一个由正文各个单词组成的列表
        :param fox_url: str
        ...
        try:
            ...
            title, contents = TfIdf().remove_punc(title, contents)
            return title, contents
        except AttributeError, e:
            print "Can't find text in url:", e
            return "", []

    def crawl_china_daily(self, china_daily_url):
        """
        提取指定chinadaily网页链接的新闻标题和正文，去掉正文中的特殊符号，返回标题的字符串和一个由正文各个单词组成的列表
        :param china_daily_url: str
        :return: (str, List[unicode])
        """
        r = requests.get(china_daily_url)
        soup = BeautifulSoup(r.text, "lxml")
        try:
    ...
    def tf_idf_10(self, urls):
        ...
        for url in urls:
            if "foxnews.com" in url:
                title, contents = TfIdf().crawl_fox_news(url)
                articles_title.append(title)
                articles_content.append(contents)
            elif "chinadaily.com" in url:
                title, contents = TfIdf().crawl_china_daily(url)
                articles_title.append(title)
                articles_content.append(contents)
            else:
                print "Can't process %s" % url
                n += 1
        ...
        for term, times in dict_df.items():
            dict_df[term] = math.log(float(len(url_list) - n) / times)
...
if __name__ == "__main__":
    url_list = ['http://www.foxnews.com/politics/2016/08/02/trump-calls-obama-worst-president-disaster-after-unfit-to-serve-slam.html',
